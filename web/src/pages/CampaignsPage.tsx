...
export function CampaignsPage() {
  ...
  return (
    <div>
      <PageHead
        title="活动投放"
        description="创建营销/事务活动，按 task_id 或 biz_id 查询进度，支持暂停、恢复、取消与失败重推。"
      />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Panel>
          <form onSubmit={onCreate}>
            <PanelTitle>创建活动</PanelTitle>
            <Field label="biz_id（幂等键）">
              <Input
                required
            ...
            <Field label="biz_scene" hint="demo/dev 使用内置 Demo 人群；txn 等可走高优映射">
              <Input
                required
            ...
            <Field
              label="模板 code（须 approved）"
              hint={templates.length === 0 ? '暂无已通过模板，请先到模板中心审核' : undefined}
            >
              <Select
                required
            ...
            <Field label="Demo 人群总量 audience_extra.total">
              <Input
                type="number"
                min={1}
            ...
            <Field label="多渠道 channels（逗号分隔，优先于 channel）">
              <Input
                className="font-mono text-sm"
